...
const app = new Hono()
  ...
  .patch("/:id", authMiddleware, zValidator("param", z.object({
    id: z.string().optional()
  })), async (c) => {
    const { id } = c.req.valid("param")
    if (!id) {
  ...
  .delete("/:id", authMiddleware, zValidator("param", z.object({
    id: z.string().optional()
  })), async (c) => {
    const { id } = c.req.valid("param")
    if (!id) {
    ...
    if (!project) {
      return c.json(
        { message: "Error deleting project!, Try again later" }, status.NOT_FOUND
      );
    }
